...
1. **Deploy the stack** with the DynamoDB on-demand fix (`BillingMode.PAY_PER_REQUEST`):
   ```bash
   ./scripts/deploy.sh -r us-west-2 -u dev-sshreyy
   ```
...
```bash
cd loadtest
UNIQUE_ID=dev-sshreyy AWS_REGION=us-west-2 node setup.mjs 120
```
...
```bash
UNIQUE_ID=dev-sshreyy AWS_REGION=us-west-2 node metrics.mjs 20
```
