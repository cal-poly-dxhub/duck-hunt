...
| `LEVEL`               | `GAME#{game_id}` | `LEVEL#{level_id}`              | `LEVEL#{level_id}` | `GAME#{game_id}`                | -                | -                   | -                  | -                   | `game_id`, `levelName`, `character`, `location`, `clues`, `easyClues`, `mapLink`, `max_tokens` | `QUERY(PK=GAME#{game_id}, SK.begins_with(LEVEL#))` (All levels for game) <br> `GET_ITEM(PK=GAME#{game_id}, SK=LEVEL#{level_id})`                                                                                                                                                                                              |
...
The access patterns remain the same since you're just adding attributes to existing entities. The DynamoDB Map and List data types handle the nested objects (`character` and `location`) efficiently.
