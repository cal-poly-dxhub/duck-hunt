...
| :-------------------- | :--------------- | :------------------------------ | :----------------- | :------------------------------ | :--------------- | :------------------ | :----------------- | :------------------ | :--------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GAME`                | `GAME#{game_id}` | `#METADATA`                     | -                  | -                               | -                | -                   | -                  | -                   | `name`, `description`, `levelsInGame`, `winner_team_id`                                        | `GET_ITEM(PK=GAME#{game_id}, SK=#METADATA)`                                                                                                                                                                                                                                                                                   |
...
| `TEAM_LEVEL`          | `TEAM#{team_id}` | `LEVEL#{level_id}`              | `LEVEL#{level_id}` | `TEAM#{team_id}`                | -                | -                   | -                  | -                   | `team_id`, `level_id`, `index`, `started_at`, `completed_at`                                    | `QUERY(PK=TEAM#{team_id}, SK.begins_with(LEVEL#))` (All levels for team) <br> `QUERY_GSI1(GSI1PK=LEVEL#{level_id})` (Teams on a level)                                                                                                                                                                                        |
| `MESSAGE`             | `USER#{user_id}` | `MESSAGE#{ts}#{id}`             | `TEAM#{team_id}`   | `MESSAGE#{ts}#{id}`             | `GAME#{game_id}` | `MESSAGE#{ts}#{id}` | `LEVEL#{level_id}` | `MESSAGE#{ts}#{id}` | `user_id`, `team_id`, `game_id`, `level_id`, `role`, `text`, `is_fallback`                                    | `QUERY(PK=USER#{user_id}, SK.begins_with(MESSAGE#))` (User messages)<br> `QUERY_GSI1(GSI1PK=TEAM#{team_id}, SK.begins_with(MESSAGE#))` (Team messages)<br> `QUERY_GSI2(GSI2PK=GAME#{game_id}, SK.begins_with(MESSAGE#))` (Game messages)<br> `QUERY_GSI3(GSI3PK=LEVEL#{level_id}, SK.begins_with(MESSAGE#))` (Level messages) |
...
- **`winner_team_id` (String, optional):** Team that finished first. Written with a
  conditional `attribute_not_exists(winner_team_id)` expression so the first finisher wins
  atomically even when several teams complete simultaneously. Absent until a team finishes.
...
#### MESSAGE Entity Additional Attributes

- **`is_fallback` (Boolean, optional):** Present and `true` when an assistant message is the
  hardcoded "technical difficulties" text rather than a model response. Fallbacks are persisted
  so chat history keeps alternating user/assistant turns, which the Converse API requires; filter
  on this attribute to exclude them from analytics.

#### TEAM_LEVEL Entity Additional Attributes

- **`index` (Number):** Position of this level in the team's route (0-based). Route order is
  randomized per team, so this — not `level_id` — defines play order.
- **`started_at` (String, optional):** ISO 8601 timestamp of the team's first interaction
  with this level. Written with a conditional `attribute_not_exists(started_at)` expression
  so only the first interaction sets it.
  > **Do not remove.** No code in this repository reads `started_at`; it is consumed by an
  > external dashboard for per-level timing. It will look unused to a static search.
- **`completed_at` (String, optional):** ISO 8601 timestamp set when the team advances past
  this level. Absent while the level is still in progress.
