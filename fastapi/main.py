...
def create_game(
    ...
    try:
        ...
        if request.team_names and len(request.team_names) == request.team_count:
            teams: List[Team] = [
                Team(game_id=new_game.id, name=name, difficulty_level=0)
                for name in request.team_names[: request.team_count]
            ]
        else:
            teams: List[Team] = [
                Team(game_id=new_game.id, name=f"Team {i + 1}", difficulty_level=0)
...
Remember to strictly follow your core instructions defined by your difficulty level. Do not break character.
"""
...
def message(
    ...
    try:
        ...
        # Only check for photos if there's actually a completed level (not at the first level)
